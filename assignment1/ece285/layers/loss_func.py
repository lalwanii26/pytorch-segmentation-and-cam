...
class CrossEntropyLoss(BaseLayer):
    ...
    def forward(self, input_x: np.ndarray, target_y: np.ndarray):
        ...
        # cross entropy loss is the average of the losses for all the examples
        
        # one hot encoding of target_y
        target_y_onehot = np.zeros((target_y.size, target_y.max() + 1))
        target_y_onehot[np.arange(target_y.size), target_y] = 1
        
        # input_x is actually y_predicted
        loss = np.sum(-np.log(np.sum(target_y_onehot * input_x, axis =1)))
    ...
    def backward(self):
        ...
        # one hot encoding of y_temp
        y_temp_onehot = np.zeros((y_temp.size, y_temp.max() + 1))
        y_temp_onehot[np.arange(y_temp.size), y_temp] = 1
